...
export function filterItemsByQuery<T extends ItemOrSkill>(
  ...
  return items.filter((item) =>
    doesQueryExistInItemOrAttributes(item, query, ignoredProperties)
  );
}
...
function doesQueryExistInItemOrAttributes(
  ...
  if (Array.isArray(item)) {
    return item.some((subItem) =>
      doesQueryExistInItemOrAttributes(subItem, query)
    );
    ...
    if (item instanceof Date) {
      ...
      return Object.keys(item).some(
        (key) =>
          ...
          doesQueryExistInItemOrAttributes(item[key], query)
      );
    }
  } else {
